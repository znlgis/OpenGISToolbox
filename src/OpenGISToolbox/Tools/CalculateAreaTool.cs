...
/// <summary>
/// Calculates the area of each feature in a vector layer.
/// </summary>
public class CalculateAreaTool : ToolBase
{
    private const string FileFilter = "Shapefile|*.shp|GeoJSON|*.geojson|GeoPackage|*.gpkg";

    public override string Id => "calculate-area";
    public override string Name => "Calculate Area";
    public override string NameZh => "计算面积";
    public override string Description => "Calculate the area of each feature in a vector layer";
    public override string DescriptionZh => "计算矢量图层中每个要素的面积";
        ...
        CancellationToken ct)
    {
        ...
        progress?.Report(L($"Calculating area for {layer.GetFeatureCount()} features...",
            $"正在计算 {layer.GetFeatureCount()} 个要素的面积..."));

        var sb = new StringBuilder();
        sb.AppendLine(L("=== Area Calculation Report ===", "=== 面积计算报告 ==="));
        sb.AppendLine($"{"FID",-10} {"Area",20}");
        ...
        foreach (var feature in layer.Features)
        {
            ...
            {
                area = GeometryUtil.AreaWkt(feature.Wkt);
            }
            totalArea += area;

            if (count < 50)
            {
                sb.AppendLine($"{feature.Fid,-10} {area,20:N6}");
            }
            count++;
        }
        ...
        sb.AppendLine(new string('-', 32));
        sb.AppendLine(L($"Total Area: {totalArea:N6} (coordinate system units\u00b2)",
            $"总面积: {totalArea:N6}（坐标系单位\u00b2）"));
